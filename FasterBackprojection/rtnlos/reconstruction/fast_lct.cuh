...
inline __global__ void scaleIntensity(cufftComplex* __restrict__ intensity, glm::uvec3 dataResolution, glm::uint numElements, float divisor)
{
    ...
    double mult = static_cast<float>(t) * weight;
    intensity[tid].x *= mult;
    intensity[tid].y *= mult;
}

inline __global__ void multiplyTransformTranspose(
    const cufftComplex* __restrict__ volumeGpu, const float* __restrict__ mtx, cufftComplex* __restrict__ mult, glm::uvec3 dataResolution)
{
    const glm::uint t = blockIdx.x * blockDim.x + threadIdx.x, y = blockIdx.y * blockDim.y + threadIdx.y, x = blockIdx.z * blockDim.z + threadIdx.z;
    if (x < dataResolution.x && y < dataResolution.y && t < dataResolution.z)
    {
...
#pragma unroll 8
        ...
        {
            sum_real += mtx[k * dataResolution.z + t] * volumeGpu[spatialIndex * dataResolution.z + k].x;
            sum_imag += mtx[k * dataResolution.z + t] * volumeGpu[spatialIndex * dataResolution.z + k].y;
        }

        mult[getKernelIdx(x, y, t, dataResolution)].x = sum_real;
        mult[getKernelIdx(x, y, t, dataResolution)].y = sum_imag;
    }
}

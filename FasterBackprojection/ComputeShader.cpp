...
GLuint ComputeShader::createShaderProgram(const char* filename)
{
	...
	std::ranges::fill(_activeSubroutineUniform[COMPUTE_SHADER], -1);		// Non valid id
...
std::vector<GLint> ComputeShader::getMaxGlobalSize()
{
	...
	glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_COUNT, 0, maxGlobalSize.data());
...
std::vector<GLint> ComputeShader::getMaxLocalSize()
{
	...
	glGetIntegeri_v(GL_MAX_COMPUTE_WORK_GROUP_SIZE, 0, maxLocalSize.data());
...
{
	return static_cast<int>(std::ceil((float)arraySize / MAX_WORK_GROUP_SIZE[axis]));
}

int ComputeShader::getWorkGroupSize(const unsigned numGroups, const unsigned arraySize)
{
	return static_cast<int>(std::ceil((float)arraySize / numGroups));
}
...
void ComputeShader::setImageUniform(const GLint id, const std::string& shaderVariable) const
{
